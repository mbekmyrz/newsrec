...
class Logger(object):
    ...
    def add_result(self, run, result):
        assert len(result) == 2 # (valid, test)
...
def print_logs(loggers: dict, log_file: str, run: int = None):
    for key in loggers.keys():
        print(key)
        loggers[key].print_statistics(run, last_best=True)
        with open(log_file, 'a') as f:
            print(key, file=f)
            loggers[key].print_statistics(run, f=f, last_best=True)
